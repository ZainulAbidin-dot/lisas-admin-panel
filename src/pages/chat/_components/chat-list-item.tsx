...
import { type Contact } from '../chat-context';

export function ChatListItem({ contact }: { contact: Contact }) {
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <Avatar className="w-10 h-10 rounded-full">
              <AvatarImage src={contact.user.profileImage} />
              <AvatarFallback>{getInitials(contact.user.name)}</AvatarFallback>
            </Avatar>
          ...
          <div>
            <p className="font-semibold text-gray-900">{contact.user.name}</p>
            <p
              className={cn(
